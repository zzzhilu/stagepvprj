        ...
        <div className="bg-gray-800/60 rounded-lg border border-gray-700 mb-3 p-3 space-y-2">
            ...
            {open && createPortal(
                <LayoutEditorModal
                    ...
                    {...{ ledLayouts, activeLedLayoutId, layout, addLedLayout, removeLedLayout, setActiveLedLayout, setLedRect, setLedResolution, addLedToLayout, removeLedFromLayout }}
                />,
                document.body
            )}
        </div>
...
// ===== 全螢幕編輯 Modal =====
function LayoutEditorModal({ leds, onClose, ledLayouts, activeLedLayoutId, layout, addLedLayout, removeLedLayout, setActiveLedLayout, setLedRect, setLedResolution, addLedToLayout, removeLedFromLayout }: any) {
    ...
    // 畫布顯示:等比縮放至最大寬 720
    const MAXW = 720;
    const scale = layout ? Math.min(MAXW / layout.canvasWidth, 460 / layout.canvasHeight) : 1;
    const dispW = layout ? layout.canvasWidth * scale : 0;
    const dispH = layout ? layout.canvasHeight * scale : 0;
