...
- 部件的 `material` **可省略**:省略 = 跟隨場館物件本身的材質。使用者在後台選取場館後,可以用「材質面板」換材質球、微調顏色/粗糙度/金屬度/反射強度(存在 `material_id` + `materialOverrides`,跟著專案同步)。
- 只有需要和主體區隔的部件才指定固定材質(例:北流的舞台面、座椅用 `blackPlastic`)。
- 新加入的場館預設 `matteGray`(霧面不反光),避免完美渲染時過曝。
